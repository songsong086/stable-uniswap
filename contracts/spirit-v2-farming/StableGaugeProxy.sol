...
contract StableGauge is ReentrancyGuard {
    ...
    IERC20 public immutable TOKEN; //LP
    address public immutable DISTRIBUTION; // contract creator
...
contract StableGaugeProxy is ProtocolGovernance, ReentrancyGuard {
        ...
        returns (address)
    {
        ...
        gauges[_tokenLP] = address(
            new StableGauge( address(rewardToken), address(governanceToken), _tokenLP, address(this))
        );
            ...
            for (uint256 i = _start; i < _end; i++) {
                ...
                if (!hasDistributed[_token] && gaugeStatus[_token]) {
                    ...
                    if (_reward > 0) {
                        ...
                        StableGauge( _gauge).notifyRewardAmount(_reward);
                    }
